...
namespace Thingspeak_Tutorial
{
    public partial class Form1 : Form
    {
        ...
        double[] rollingTemps = new double[averagesSize];
        double[] rollingLightLevel = new double[averagesSize];
        double avgLight;
        double avgTemp;
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (!string.Equals(current_data_text.Text, ""))
            {
                ...
                try
                {
                    ...
                    string strField1 = string.Format("{0:00.0}%", avgLight);
                    string strField2 = string.Format("{0:00.0}", avgTemp);
        ...
        private void serialPort1_DataReceived(object sender, System.IO.Ports.SerialDataReceivedEventArgs e)
        {
            ...
            if (RxString.Length > 0)
            {
                ...
                thisLight = (double) int.Parse(strs[0]);
                thisLight = (thisLight - 500) / 6;
                thisLight = (thisLight < 0) ? 0 : (thisLight > 100) ? 100 : thisLight;
                thisTemp = (double) int.Parse(strs[1]);
                thisTemp = (thisTemp - 230.0) / 3.6;
        ...
        private void calculateAverages(double light, double temp)
        {
            ...
            avgLight = sumLight / (double) averagesSize;
            avgTemp = sumTemp / (double)averagesSize;

            // update the labels
            if (thisRollingIdx % 2 == 0)
            {
                currentAverageLight.Text = string.Format("{0:00.0}%", avgLight);
                currentAverageTemp.Text = string.Format("{0:00.0} °F", avgTemp);
            }
        }
